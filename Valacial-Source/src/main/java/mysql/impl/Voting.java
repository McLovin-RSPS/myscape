...
public class Voting extends MotivoteHandler<Vote> {
	...
	public static void handleKilledVotingBoss(Player p) {
		...
		//TeleportHandler.teleportPlayer(p, new Position(1647, 3673), p.getSpellbook().getTeleportType());
		VOTES = 0;
			
		}
	public static void handleVotingBoss(Player p) {
		if (p.getLocation() == Location.RAID || p.getLocation() == Location.DUEL_ARENA) {
		...
		if(VOTES >= 15) {
			...
			TeleportHandler.teleportPlayer(p, new Position(3702, 2971,0), p.getSpellbook().getTeleportType());
			//VOTES = 0;
			
		} else if (VOTES <= 15) {
			p.getPacketSender().sendMessage("<col=900000>100 Votes Is Needed To Start This Event!");
		 }
	...
	public static void handleQueuedReward(Player p) {
		...
		if(VOTES == 100) {
			NPC npc = new NPC(6305, new Position(3702, 2971,0));
			World.register(npc);
		}
			if(VOTES == 15) {
