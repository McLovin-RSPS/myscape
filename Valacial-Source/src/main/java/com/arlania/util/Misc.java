...
public class Misc {
	...
	public static boolean superiorCheck(Player player, Player target) {
		switch (player.getRights()) {
			case OWNER:
			case DEVELOPER:
				return true;
			case ADMINISTRATOR:
				return target.getRights() != PlayerRights.OWNER
						&& target.getRights() != PlayerRights.ADMINISTRATOR && target.getRights() != PlayerRights.DEVELOPER;
			case MODERATOR:
				return target.getRights() != PlayerRights.OWNER
						&& target.getRights() != PlayerRights.ADMINISTRATOR
						&& target.getRights() != PlayerRights.MODERATOR && target.getRights() != PlayerRights.DEVELOPER;
			case SUPPORT:
				return target.getRights() != PlayerRights.OWNER
						&& target.getRights() != PlayerRights.ADMINISTRATOR
						&& target.getRights() != PlayerRights.MODERATOR && target.getRights() != PlayerRights.SUPPORT
						&& target.getRights() != PlayerRights.DEVELOPER;
		}
		return true;
	}
	...
	public static String setupMoney(long quantity) {
		...
		return currency(quantity) + " (" + formatLong(quantity) + " 1B Scrolls)";
	}
	/**
	 * Formats numerals
	 * 
	...
	private static final String[] BLOCKED_WORDS = new String[]{
		...
		"soulspawn", "atiloc", "<img", "@cr", "i k o v", "ik0v", "<img=", "@cr", ":tradereq:", ":instancereq:",":duelreq:",
		"<col=", "<shad=", "hostilityps", "hostilityp", "hostility ps",	"nigger","�diot","dick","pussy","boobs",
		
		/* Advertisement basics */
		"www",".com",".org",".net",".io",".ps",".tk","dotcom","dotorg","dotnet","dottk",
		
		/* Individual server names */
		"kratos","atlas","osscape","alora","elkoy","osrune","guthixp","dawntained","locopk","imagineps","nearreal","pkhonor","dreamsc","manicps","imagineps",
		"draganoth","alosps","rsps2","lostisle","necrotic","redrune","deathwish","pkowned","osbase","beastpk","roatpk","rsgenesis","trinityps","boxrune","runique",
		"furiousp","novus","ikov","joinmy","atarax","nardahp","illerai","letspk","ratedpixel","cloudnine",	"viceos","deprivedr","exoria","simplicityp","cruxp",
		"ospkz","scapewar","amberp","diviner","osunity","amulius","zenyteps","zenyteosrs"};
