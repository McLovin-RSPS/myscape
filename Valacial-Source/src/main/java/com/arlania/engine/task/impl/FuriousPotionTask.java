...
public class FuriousPotionTask extends Task {
	...
	public void execute() {
		...
		if(player.getFuriousPotionTimer() <= 0 || player.getLocation() == Location.DUEL_ARENA || player.getLocation() == Location.RAID || player.getLocation() == Location.WILDERNESS && player.getRights() != PlayerRights.DEVELOPER) {
