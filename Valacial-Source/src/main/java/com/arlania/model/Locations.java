...
import com.arlania.world.content.minigames.impl.ExecutiveZone;
...
import com.arlania.world.content.minigames.impl.SponsorZone;
...
public class Locations {
	...
	public enum Location {
        ...
        GODS_RAID(new int[] { 3034,  3071 }, new int[] { 2705, 2743 }, true, false,
                false, false, false, false) {
            @Override
            public void enter(Player player) {
                World.minigameHandler.getMinigame(0).handleLocationEntry(player);
                player.sendMessage("You just joined OP raid!");
            }

            @Override
            public void leave(Player player) {
                World.minigameHandler.getMinigame(0).handleLocationOutry(player);
				player.sendMessage("You just left OP raid!");

			}
        },    KEYMG(new int[] { 2570,  2595 }, new int[] {4608, 4620 }, true, false,
                false, false, false, false) {
            @Override
            public void enter(Player player) {
                player.sendMessage("welcome to key minigame!!");
            }

        },
        RAID(new int[]{3433, 3459, 2421, 2499}, new int[]{3694, 3729, 4915, 4990}, true, false, true, false, true, false) {
			@Override
		...
		WILDERNESS(new int[]{2941, 3392, 2986, 3012, 3653, 3710, 3650, 3653}, new int[]{3523, 3968, 10338, 10366, 3441, 3538, 3457, 3472}, false, true, true, true, true, true) {
			...
			@Override
			public boolean canTeleport(Player player) {//lmao ishall have delete his whole files of desktop
...
//		SHREK(new int[]{2645, 2661}, new int[]{3027, 3046}, true, true, true, false, true, true) {
//			@Override
//			public void login(Player player) {
//				Shrek.leaveZone(player);
//			}
//
//			@Override
//			public void enter(Player player) {
//				Shrek.enterZone(player);
//			}
//
//			@Override
//			public void process(Player player) {
...
//
//			@Override
//			public void leave(Player player) {//wrong coords 100%
//				Shrek.leaveZone(player);
//			}
//
//
...
//
//		},
		EXECUTIVE_ZONE(new int[]{2895, 2940}, new int[]{2770, 2815}, false, false, false, false, false, false) {
			@Override
			public void login(Player player) {
				ExecutiveZone.leaveZone(player);
			}
			
			@Override
			public void enter(Player player) {
				ExecutiveZone.enterZone(player);
			}
		...
		SPONSOR_ZONE(new int[]{2690, 2740}, new int[]{2958, 3005}, false, false, false, false, false, false) {
			@Override
			public void login(Player player) {
				SponsorZone.leaveZone(player);
			}
			
			@Override
			public void enter(Player player) {
				SponsorZone.enterZone(player);
			}
		...
		Location(int[] x, int[] y, int z,boolean multi, boolean summonAllowed, boolean followingAllowed, boolean cannonAllowed, boolean firemakingAllowed, boolean aidingAllowed) {
			this.x = x;
			this.y = y;
			this.z = z;
			this.multi = multi;
		...
		public static boolean inMulti(Character gc) {
			if(gc.getLocation() == WILDERNESS) {
				...
				if(x >= 3250 && x <= 3302 && y >= 3905 && y <= 3925 || x >= 3090 && x <= 3140 && y >= 3809 && y <= 3850 || x >= 3650 && y <= 3538 || x >= 3020 && x <= 3055 && y >= 3684 && y <= 3711 || x >= 3150 && x <= 3195 && y >= 2958 && y <= 3003 || x >= 3040 && x <= 3071 && y >= 5215 && y <= 5184)
			...
			if (x >= 2450 && x <= 2660 && y >= 10100 && y <= 10170) {
				return true;
			}
			if (x >= 3039 && x <= 3075 && y >= 5182 && y <= 5211) {
			   return true;// indians
			}
			if (x >= 2773 && x <= 2811 && y >= 3760 && y <= 3805) {
				   return true;// musketir
			}
			if (x >= 3011 && x <= 3039 && y >= 5217 && y <= 5247) {
				   return true;// Death Reaper
			}
			if (x >= 2897 && x <= 2933 && y >= 2772 && y <= 2811) {
				   return true;// Ex Zone
			}
			if (x >= 1620 && x <= 1645 && y >= 4695 && y <= 4714) {
				   return true;// thanos
			}
			if (x >= 2884 && x <= 2940 && y >= 2749 && y <= 2695) {
				   return true;// madara
			}
			if (x >= 2570 && x <= 2612 && y >= 2572 && y <= 2605) {
				   return true;// lucifer
			}
			if (x >= 2049 && x <= 2106 && y >= 4798 && y <= 4870) {
				   return true;// angelic
			}
			if (x >= 3016 && x <= 3050 && y >= 2878 && y <= 2913) {
				   return true;// sephir
			}
			if (x >= 1855 && x <= 1916 && y >= 4488 && y <= 4525) {
				   return true;// infinity
			}
			if (x >= 3265 && x <= 3322 && y >= 4289 && y <= 4335) {
				   return true;// corp
			}
			if (x >= 2456 && x <= 2496 && y >= 4770 && y <= 4800) {
				   return true;// chaos
			}
			if (x >= 1724 && x <= 1796 && y >= 5141 && y <= 5258) {
				   return true;// diglet
			}
			if (x >= 3008 && x <= 3071 && y >= 5312 && y <= 5375) {
				   return true;// skotizo
			}
			if (x >= 2840 && x <= 2874 && y >= 3712 && y <= 3744) {
				   return true;// skotizo
			}
...
//		if (newLocation == Location.DEFAULT) {
//			if (gc.getPosition().getX() >= 2645 && gc.getPosition().getX() <= 2661 && gc.getPosition().getY() >= 3027 && gc.getPosition().getY() <= 3046) {
//				newLocation = Location.SHREK;
//			}
//		}
		if (newLocation == Location.DEFAULT) {
			if (gc.getPosition().getX() >= 2895 && gc.getPosition().getX() <= 2940 && gc.getPosition().getY() >= 2770 && gc.getPosition().getY() <= 2815) {
				newLocation = Location.EXECUTIVE_ZONE;
			}	
		}
		if (newLocation == Location.DEFAULT) {
			if (gc.getPosition().getX() >= 2690 && gc.getPosition().getX() <= 2740 && gc.getPosition().getY() >= 2958 && gc.getPosition().getY() <= 3005) {
				newLocation = Location.SPONSOR_ZONE;
			}	
		}
//		System.err.println(newLocation);//TODO
