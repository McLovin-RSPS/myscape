...
public class PuroPuro {
	...
	public static void catchImpling(Player player, final NPC imp) {
		...
		if(player.getInterfaceId() > 0 || player == null || imp == null || implingData == null || !imp.isRegistered() || !player.getClickDelay().elapsed(2000))
			return;
		if(player.getSkillManager().getCurrentLevel(Skill.ARCHAEOLOGY) < implingData.levelReq) {
		...
		boolean sucess = player.getSkillManager().getCurrentLevel(Skill.ARCHAEOLOGY) > 8 ? Misc.getRandom(player.getSkillManager().getCurrentLevel(Skill.ARCHAEOLOGY) / 2) > 1 : true;
		if(sucess) {
			if(imp.isRegistered()) {
				...
				player.getSkillManager().addExperience(Skill.ARCHAEOLOGY, implingData.XPReward);
				if (implingData == ImpData.YOUNG) {
//					//player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.CATCH_A_YOUNG_IMPLING, 1);
//				else if(implingData == ImpData.KINGLY) {
//					player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.CATCH_5_KINGLY_IMPLINGS, 1);
//					player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.CATCH_100_KINGLY_IMPLINGS, 1);
//
//				}
				}
			}
