...
import com.arlania.world.content.minigames.Minigame2;
...
public abstract class Raid extends Minigame2 {
	...
	protected Position endPos = new Position(3817, 3484);

	// delays
	public static final int START_DELAY = 10; // 120 seconds
	...
	protected void processActive() {
		...
		if (stages[currentStage].isFinished() && currentStage == stages.length - 1) {
			giveRewards();
			sendLocalMessage("well done finishing " + getName());
		...
		if (playerIndex == 0) {
			World.sendMessage("@red@[Raids] <col=16777215>" + getName() + " ended with no winners");
			GodsRaid.setStarted(false);
			init();
			return;
		}
		if (playerIndex == 1 && !lastPlayerAnnouncement) {
			World.sendMessage("@red@[Raids] <col=16777215>" + players[0].getUsername() + " is left alone in " + getName());
	...
	protected void processInActive() {
		...
		if (playerIndex > 0) {
			...
			if (!aboutToStartAnnouncement) {
				World.sendMessage("@red@[Raids] @bla@" + getName() + " is starting in " + START_DELAY + " seconds");
	...
	public boolean handleNpcDeath(NPC npc) {
		...
		for (int i = 0; i < playerIndex; ++i) {
			...
			//		players[i].getInventory().add(7118,1);
					//World.sendMessage("<shad=0>@bla@[@mag@Donator Box@bla@]@mag@ "+players[i].getUsername()+ "@bla@ has just received a @mag@Donator Box!@bla@");
