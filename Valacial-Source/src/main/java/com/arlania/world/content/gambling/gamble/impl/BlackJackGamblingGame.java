...
public class BlackJackGamblingGame extends GamblingGame {
	...
	public void gamble() {
		...
		getHost().getGambling1().setSelection(-1);
		getOpponent().getGambling1().setSelection(-1);
				...
				case 0:
					if (!isHostTurn()) {
						if (opponentScore == 0) {
							...
							selection = getOpponent().getGambling1().getSelection();
						}
					} else if (isHostTurn()) {
						if (hostScore == 0) {
							selection = 1;
							...
							selection = getHost().getGambling1().getSelection();
						}
					}
					break;
				case 1:
