	...
	public static void handle(Player player, int id) {
		...
		if(id == FIRST_OPTION_OF_FIVE) {
			...
			case 10:
				...
				break;
				case 117:
					if(player.getInventory().getAmount(5022) > 6000) {
						int random =  Misc.getRandom((10000));
						player.getInventory().delete(5022,6000);
						...
						player.sendMessage("You need 6t!");
...
//					break;
			...
			case 2:
				...
				break;
				case 117:
					if(player.getInventory().getAmount(5022) > 12000) {
						int random =  Misc.getRandom((10000));
						player.getInventory().delete(5022,12000);

						NPC bandosava = new NPC(6303, new Position(2528, 5175, random*100+8));
						bandosava.getDefinition().setRespawnTime(5);
						World.register(bandosava);
						bandosava = new NPC(6303, new Position(2504, 5174, random*100+8));
						bandosava.getDefinition().setRespawnTime(5);
						...
						player.sendMessage("You need 12t!");
						player.resetInterfaces();
					}
					break;
			case 9:
			...
			case 1:
				...
				break;
				case 117:
					if(player.getInventory().getAmount(5022) > 18000) {
						int random =  Misc.getRandom((10000));
						NPC bandosava = new NPC(1234, new Position(1236, 1241, random*100+8));
						bandosava.getDefinition().setRespawnTime(5);
						World.register(bandosava);
						...
						player.sendMessage("You need 18t!");
						player.resetInterfaces();
					}
					break;
			case 2:
			...
			case 13:
				...
				player.getPacketSender().sendMessage("Speak To Drapht to obtain this rank..");
...
//				MySQLController.getStore().claim(player);
			...
			case 5:
				...
				if(player.getRights() == PlayerRights.PLAYER) {
					player.getPacketSender().sendMessage("You need to be a member to teleport to this zone.").sendMessage("To become a member, visit ImaginePS.com and purchase a scroll.");
					return;
				}
			...
			case 9:
			case 13:
			case 26://im not advertising dw haha:)
			case 27:
			case 28:
			case 41:
				player.getPacketSender().sendInterfaceRemoval();
			...
			case 31:
				...
				break;
			case 45:
				player.getPacketSender().sendString(1, "ImaginePS.org");
				break;
			}
			...
			switch(player.getDialogueActionId()) {
			 ...
			 case 280:
                 player.setTutorialStage(TutorialStages.AFK_PLACE_START);
                 TutorialStages.AFK_PLACE_START.sendDialogueText(player);
                 break;
			case 666:
				final Item fee = new Item(995, 5000000);
				
				if (player.getInventory().hasItem(fee))
                    player.getInventory().delete(fee).refreshItems();
                else if(fee.getId() == 995 && player.getMoneyInPouch() >= fee.getAmount()) {
                    player.setMoneyInPouch(player.getMoneyInPouch() - fee.getAmount());
                    player.updateMoneyPouch();
                }else {
                    player.getPacketSender().closeAllWindows();
                    player.sendMessage("@red@You need " + fee.getAmount() + "x " + fee.getDefinition().getName() + " in order to enter.");
                    return;
                }
				player.getPacketSender().closeAllWindows();
			...
			case 66:
				player.getPacketSender().sendInterfaceRemoval();
				if(player.getLocation() == Location.RAID && player.getMinigameAttributes().getDungeoneeringAttributes().getParty() == null) {
			...
			case 5:
				player.getPacketSender().sendInterfaceRemoval();
				if(player.getRights() == PlayerRights.PLAYER) {
					player.getPacketSender().sendMessage("You need to be a member to teleport to this zone.").sendMessage("To become a member, visit ImaginePS.org and purchase a scroll.");
					return;
				}
				TeleportHandler.teleportPlayer(player, new Position(3424, 2919), player.getSpellbook().getTeleportType());
				break;
...
//				break;
			...
			case 71:
				...
				if(player.getInventory().contains(19670) && player.getClickDelay().elapsed(700)) {
					...
					if(player.getGameMode() == GameMode.NORMAL) {
						player.getInventory().add(5022, 3000 * amt);
					} else {
						player.getInventory().add(5022, 5000 * amt);
					}
					player.getPacketSender().sendMessage("You claim the "+(amt > 1 ? "scrolls" : "scroll")+" and receive your reward.");
					player.getClickDelay().reset();
				}
