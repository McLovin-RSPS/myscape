...
public class FlowerPokerGamblingGame extends GamblingGame {
	...
	private static Ranking getRank(Player player) {
		 ...
		 */
		flowers.addAll(player.getGambling1().getFlowers());
		/**
		 * Sorts the list
		 */
	...
	private void plant(Player player, Player opponent) {
		...
		TaskManager.submit(new Task(1, false) {
			...
			int type = 0;
			Flowers flower = player.getGambling1().getFlowers().get(type);
			...
			protected void execute() {
				...
				case 2:
					 ...
					 * The flower
					 */
					flower = player.getGambling1().getFlowers().get(type);
					/**
					 * No flower
					 */
					if (flower == null) {
					 ...
					 */
					player.getGambling1().getGameFlowers().add(gameFlower);
					/**
	...
	private Player getOtherPlayer(Player player) {
		...
		return player.getGambling1().getGame().getHost() == player ? player.getGambling1().getGame().getOpponent()
				: player.getGambling1().getGame().getHost();
	}
	...
	public void gamble() {
		 ...
		 * Resets flowers
		 */
		getHost().getGambling1().getFlowers().clear();
		getOpponent().getGambling1().getFlowers().clear();
		/**
		 * Resets game flowers
		 */
		getHost().getGambling1().getGameFlowers().clear();
		getOpponent().getGambling1().getGameFlowers().clear();
		/**
		 * Gets the flowers
		 */
		for (int i = 0; i < 5; i++) {
			...
			getHost().getGambling1().getFlowers().add(hostFlower);
			getOpponent().getGambling1().getFlowers().add(opponentFlower);
		}
		/**
		 * The host result
		 */
