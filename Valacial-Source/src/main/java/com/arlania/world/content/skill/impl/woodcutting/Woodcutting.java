						...
						player.setCurrentTask(new Task(1, player, false) {
							...
							public void execute() {
								...
								if (cycle == reqCycle) {
									...
									if (!t.isMulti() || Misc.getRandom(100) == 1 && object.getId() != 9222) {
										//player.performAnimation(new Animation(65535));
										if (object.getId() == 11434) {
											return;
										}
										...
										if (t == Trees.EVIL_TREE) {
											...
											player.getPacketSender().sendMessage("You get "+(t == Trees.AFK_TREE ? "a afk ticket..." : "some logs..."));
										}
									}
									...
									if(t == Trees.OAK) {
										player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.CUT_AN_OAK_TREE, 1);

									} else if(t == Trees.MAGIC) {
										player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.CUT_100_MAGIC_LOGS, 1);
										player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.CUT_5000_MAGIC_LOGS, 1);
