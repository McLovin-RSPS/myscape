...
import com.arlania.engine.task.impl.FuriousPotionTask;
...
public class Consumables {
	...
	private static void eat(Player player, FoodType food, int slot) {
		...
		//	player.getPotionTimer().reset();
			...
			} else if(food == FoodType.ROCKTAIL) {
				player.getAchievementTracker().progress(AchievementData.EAT_A_ROCKTAIL, 1);
	...
	private enum FoodType {
		...
		RAINBOW_FISH(new Item(10136), 270),
		/*
	...
	public static void handlePotion(final Player player, final int itemId, final int slot) {
		...
		if (player.getPotionTimer().elapsed(900)) {
			...
			case 14291:
				...
				break;
			case 4460:
				if(!drinkFuriousflask(player, slot, 4462))
					return;
				break;
			case 4462:
				if(!drinkFuriousflask(player, slot, 4464))
					return;
				break;
			case 4464:
				if(!drinkFuriousflask(player, slot, 4466))
					return;
				break;
			case 4466:
				if(!drinkFuriousflask(player, slot, 14207))
					return;
				break;
			}
			player.getPacketSender().sendInterfaceRemoval();
	...
	public static boolean drinkOverload(final Player player, int slot, int replacePotion) {
		...
		if(player.getFuriousPotionTimer() > 0) {
			player.getPacketSender().sendMessage("It would be dangerous to take a Overload potion right now.");
			return false;
		}
	...
	public static void furiousIncrease(final Player player, Skill skill, double l) {
		player.getSkillManager().setCurrentLevel(skill, (int) (SkillManager.getLevelForExperience(player.getSkillManager().getExperience(skill)) + (SkillManager.getLevelForExperience(player.getSkillManager().getExperience(skill)) * l)), true);
	}
	
	public static boolean drinkFuriousflask(final Player player, int slot, int replacePotion) {
		if(player.getLocation() == Location.WILDERNESS || player.getLocation() == Location.DUEL_ARENA) {
			player.getPacketSender().sendMessage("You cannot use this potion here.");
			return false;
		}
		if(player.getFuriousPotionTimer() > 0) {
			player.getPacketSender().sendMessage("You already have the effect of a Furious potion.");
			return false;
		}
		if(player.getOverloadPotionTimer() > 0) {
			player.getPacketSender().sendMessage("It would be dangerous to take an overload potion right now.");
			return false;
		}
		player.performAnimation(new Animation(829));
		player.getInventory().getItems()[slot] = new Item(replacePotion, 1);
		player.getInventory().refreshItems();
		player.setFuriousPotionTimer(600);
		TaskManager.submit(new FuriousPotionTask(player));
		return true;
	}
