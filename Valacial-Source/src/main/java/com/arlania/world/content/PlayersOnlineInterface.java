...
public class PlayersOnlineInterface {
	...
	public final static CopyOnWriteArrayList<Player> PLAYERS_ONLINE_LIST = new CopyOnWriteArrayList<Player>();
	...
	public static int getPlayer(Player player){
		int index = 0;
		for(Player plr : PLAYERS_ONLINE_LIST) {
			if(plr == player) {
				return index;
			}
			index++;
	...
	private static int getValue(Player p) {
		...
		switch(p.getRights()) {
			...
			case YOUTUBER:
				value = 8;
				break;
			case VETERAN: //TESTING
				value = 14;
				break;
			case PARTNER_DONATOR: //TESTING
				value = 15;
				break;
			case CONTRIBUTER: //TESTING
				value = 12;
				break;
		}
		if(value == 0) {
