...
public class Hunter {
	...
	public static boolean goodArea(Player client) {
		int x = client.getPosition().getX();
		int y = client.getPosition().getY();
		return x >= 2758 && x <= 2965 && y >= 2880 && y <= 2954 || x >= 3174 && x <= 3196 && y >= 3924 && y <= 3944;
	}
	...
	public static int getMaximumTraps(Player client) {
		return client.getSkillManager().getCurrentLevel(Skill.ARCHAEOLOGY) / 20 + 1;
	}
	...
	public static void layTrap(Player client, Trap trap) {
		...
		if (trap instanceof BoxTrap) {
			id = 10008;
			if(client.getSkillManager().getCurrentLevel(Skill.ARCHAEOLOGY) < 60) {
		...
		if (canLay(client)) {
			...
			if (trap instanceof SnareTrap) {
				...
				if (client.getSkillManager().getCurrentLevel(Skill.ARCHAEOLOGY) < 27) {
					...
					if (theTrap.getOwner() == client) {
						if (theTrap instanceof SnareTrap) {
							...
							if (theTrap.getGameObject().getId() == 19180) {
								...
								client.getSkillManager().addExperience(Skill.ARCHAEOLOGY, exps[0] );
								...
								client.getSkillManager().addExperience(Skill.ARCHAEOLOGY, exps[1]);
								...
								client.getSkillManager().addExperience(Skill.ARCHAEOLOGY, exps[2]);
								...
								client.getSkillManager().addExperience(Skill.ARCHAEOLOGY, (exps[3]));
								...
								client.getSkillManager().addExperience(Skill.ARCHAEOLOGY, exps[4]);
							}
						} else if (theTrap instanceof BoxTrap) {
							client.getInventory().add(10008, 1);
							if (theTrap.getGameObject().getId() == 19191) {
								client.getInventory().add(10033, 1);
								client.getSkillManager().addExperience(Skill.ARCHAEOLOGY, exps[6]);
								client.getPacketSender().sendMessage(
										"You've succesfully caught a chinchompa!");
							} else if (theTrap.getGameObject().getId() == 19189) {
								client.getInventory().add(10034, 1);
								client.getSkillManager().addExperience(Skill.ARCHAEOLOGY, exps[7]);
								client.getPacketSender()
								.sendMessage(
	...
	public static void catchNPC(Trap trap, NPC npc) {
		if (trap.getTrapState().equals(TrapState.CAUGHT))
			return;
		if (trap.getOwner() != null) {
			if (trap.getOwner().getSkillManager().getCurrentLevel(Skill.ARCHAEOLOGY) < requiredLevel(npc.getId())) {
				trap
				.getOwner()
				.getPacketSender()
				.sendMessage(
