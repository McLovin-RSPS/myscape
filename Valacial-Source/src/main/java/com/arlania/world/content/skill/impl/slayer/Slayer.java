...
public class Slayer {
		...
		if(slayerTask != SlayerTasks.NO_TASK) {
			if(slayerTask.getNpcId() == npc.getId()) {
				...
				if(Misc.inclusiveRandom(1,75) == 1) {
					NPCDrops.drop(player, new Item(2717, 1), npc, npc.getPosition(), false);
				}
				if(Misc.inclusiveRandom(1,75) == 1) {
					NPCDrops.drop(player, new Item(20900, 1), npc, npc.getPosition(), false);
				}
	...
	public void handleSlayerTaskDeath(boolean giveXp) {
		...
		if(amountToSlay > 1) {
			...
			taskStreak++;
			player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.COMPLETE_A_SLAYER_TASK, 1);
			player.getAchievementTracker().progress(AchievementData.COMPLETE_10_SLAYER, 1);

			if(slayerTask.getTaskMaster() == SlayerMaster.KURADEL) {
				player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.COMPLETE_A_HARD_SLAYER_TASK, 1);
			} else if(slayerTask.getTaskMaster() == SlayerMaster.SUMONA) {
				player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.COMPLETE_AN_ELITE_SLAYER_TASK, 1);
