...
public class DeathCombat implements CombatStrategy {
	...
	public boolean customContainerAttack(Character entity, Character victim) {
		...
		if(Locations.goodDistance(musketier.getPosition().copy(), victim.getPosition().copy(), 1) && Misc.getRandom(5) <= 3) {
			...
			((Player)victim).getPacketSender().sendGlobalGraphic(new Graphic(910), pos);
			musketier.performAnimation(new Animation(711));
			musketier.forceChat("I Will Take You to your Grave!");
			...
			musketier.performAnimation(new Animation(barrage ? 711 : 711));
			musketier.forceChat("I am Going To kill you!");
				...
				public void execute() {
					...
					//	new Projectile(musketier, victim, 89, 44, 3, 43, 43, 0).sendProjectile();
						...
						if(barrage && Misc.getRandom(6) <= 3) {
							musketier.performAnimation(new Animation(711));
							musketier.forceChat("DIEEEE!");
							for(Player toAttack : Misc.getCombinedPlayerList((Player)victim)) {
								if(toAttack != null && Locations.goodDistance(musketier.getPosition(), toAttack.getPosition(), 7) && toAttack.getConstitution() > 0) {
									...
									toAttack.performGraphic(new Graphic(1639));
									musketier.forceChat("Death awaits you!");
								}
							}
						}
