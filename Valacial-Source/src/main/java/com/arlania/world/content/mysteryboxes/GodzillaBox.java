 ...
 * @author Tibo
 */
public class GodzillaBox {

	/**
	 * The player object that will be triggering this event
	 */
	private final Player plr;
	private final int BOX = 7104;
	...
	public void process() {
		...
		if (player.getInventory().hasItem(new Item(BOX, 1))) {
			...
			if (randomYoda == 0) {
				...
				World.sendMessage("<shad=0>@bla@[@red@Pokeball@bla@] [@red@Dream@bla@] @red@" + player.getUsername() + "@bla@ Has just received a @red@ " + ItemDefinition.forId(godzilla[rewardGrade3][rewardPos]).getName() + " @bla@!");
				...
				if (randomDouble == 0) {
					...
					player.setAnimation(new Animation(6382));
					player.setGraphic(new Graphic(127));
					World.sendMessage("<shad=0>@bla@[@or1@Pokeball@bla@] [@or1@Rare@bla@] @or1@" + player.getUsername() + "@bla@ Has just received a @or1@ " + ItemDefinition.forId(godzilla[rewardGrade3][rewardPos]).getName() + " @bla@!");
					double randomDouble = Misc.random(125);
				...
				player.setAnimation(new Animation(6382));
				player.setGraphic(new Graphic(127));
				World.sendMessage("<shad=0>@bla@[@gre@Pokeball@bla@] [@gre@common@bla@] @gre@" + player.getUsername() + "@bla@ Has just received a @gre@ " + ItemDefinition.forId(godzilla[rewardGrade3][rewardPos]).getName() + " @bla@!");
				double randomDouble = Misc.random(125);
