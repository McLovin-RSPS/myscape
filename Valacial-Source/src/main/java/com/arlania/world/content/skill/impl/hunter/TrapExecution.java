...
public class TrapExecution {
	...
	public static int successFormula(Trap trap, NPC npc) {
		...
		if (Hunter.hasLarupia(trap.getOwner()))
			chance = chance + 10;
		chance = chance
				+ (int) (trap.getOwner().getSkillManager().getCurrentLevel(Skill.ARCHAEOLOGY) / 1.5)
				+ 10;

		if (trap.getOwner().getSkillManager().getCurrentLevel(Skill.ARCHAEOLOGY) < 25)
			chance = (int) (chance * 1.5) + 8;
		if (trap.getOwner().getSkillManager().getCurrentLevel(Skill.ARCHAEOLOGY) < 40)
			chance = (int) (chance * 1.4) + 3;
		if (trap.getOwner().getSkillManager().getCurrentLevel(Skill.ARCHAEOLOGY) < 50)
			chance = (int) (chance * 1.3) + 1;
		if (trap.getOwner().getSkillManager().getCurrentLevel(Skill.ARCHAEOLOGY) < 55)
			chance = (int) (chance * 1.2);
		if (trap.getOwner().getSkillManager().getCurrentLevel(Skill.ARCHAEOLOGY) < 60)
			chance = (int) (chance * 1.1);
		if (trap.getOwner().getSkillManager().getCurrentLevel(Skill.ARCHAEOLOGY) < 65)
