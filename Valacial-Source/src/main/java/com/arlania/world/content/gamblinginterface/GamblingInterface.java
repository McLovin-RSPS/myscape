    ...
    public void gambleItem(int itemId, int amount, int slot) {
        ...
        if (player.getRights() != PlayerRights.DEVELOPER) {
            if (!new Item(itemId).tradeable()) {
                player.getPacketSender().sendMessage("This item can not be gambled.");
                return;
            }
        }
        falseGambleConfirm();
...
//done
    ...
    private void start55x2() {
        ...
        player2.getPacketSender().removeAllWindows();

        player.moveTo(new Position(3028, 2908));
        player2.moveTo(new Position(3028, 2907));
    ...
    private void startDiceDuel() {
        ...
        player.moveTo(new Position(3028, 2902));
        player2.moveTo(new Position(3028, 2901));
        player.setDirection(Direction.EAST);
        player2.setDirection(Direction.WEST);
    ...
    private void sendDescriptionForMode(GamblingMode mode) {
        ...
        if (mode.equals(GamblingMode.DICE_DUEL)) {
            player.getPacketSender().sendString(57157, "@whi@The player who has the highest")
                    .sendString(57158, "@whi@roll gets the round win.")
                    .sendString(57159, "@whi@First player to win 3 rounds wins the pot.");
        } else if (mode.equals(GamblingMode.FLOWER_POKER)) {
            player.getPacketSender().sendString(57157, "@whi@Both players will plant 5 flowers.")
                    .sendString(57158, "@whi@The player with the most pairs wins.")
                    .sendString(57159, "@whi@Automatic replant for draws.");
        } else if (mode.equals(GamblingMode.BLACKJACK)) {
            player.getPacketSender().sendString(57157, "@whi@Closest to 100 without busting.")
                    .sendString(57158, "@whi@Host rolls second.")
                    .sendString(57159, "@whi@Closest number wins.");
        } else if (mode.equals(GamblingMode.FIFTY_FIVE_X2)) {
            player.getPacketSender().sendString(57157, "@whi@The host will roll the dice...")
                    .sendString(57158, "@whi@If the host doesn't roll over 55")
                    .sendString(57159, "@whi@then the host wins the pot.");
        }
    ...
    private void startFlowerPoker() {
        ...
        player.moveTo(new Position(3033, 2894));
        player2.moveTo(new Position(3033, 2893));
    ...
    private void startBlackjack() {
        ...
        player.moveTo(new Position(3033, 2898));
        player2.moveTo(new Position(3033, 2897));
    ...
    private void resetData() {
        ...
        player.getPacketSender().sendString(57157, "Please select a game mode...").sendString(57158, "").sendString(57159, "");
    }
