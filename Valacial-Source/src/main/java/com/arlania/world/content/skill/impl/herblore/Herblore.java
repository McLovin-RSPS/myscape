...
public class Herblore {
		...
		if(player.getInventory().contains(pot.getUnfinishedPotion()) && player.getInventory().contains(pot.getItemNeeded())) {
			...
			TaskManager.submit(new Task(1, player, false) {
				public void execute() {
					...
					player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.MIX_A_POTION, 1);
	...
	enum SpecialPotion {
		...
		FURIOUS_POTION(new Item[] {new Item(15332), new Item(4419), new Item(4421), new Item(4468)}, new Item(4460), 98, 18103);
	...
	public static void handleSpecialPotion(Player p, int item1, int item2) {
		...
		if(specialPotData == SpecialPotion.OVERLOAD) {
			p.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.MIX_AN_OVERLOAD_POTION, 1);
			p.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.MIX_100_OVERLOAD_POTIONS, 1);
		}
	}
}
