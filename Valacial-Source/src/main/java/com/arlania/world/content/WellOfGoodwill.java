...
public class WellOfGoodwill {
	...
	public static String BOOST = "@red@N/A";
...
//		}
		...
		player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.FILL_WELL_OF_GOODWILL_1M, amount);
		...
		if (getMissingAmount() <= 0) {
			...
			BOOST = "@gre@2X Droprate";
			World.sendMessage("@red@[Well of Wealth]@bla@Is now granting everyone 1 hour of x2 Easier Drop Rates");
		}
		World.getPlayers().forEach(p -> p.getPacketSender().sendString(50865, "Well: @yel@" + WellOfGoodwill.BOOST + ""));
	}
