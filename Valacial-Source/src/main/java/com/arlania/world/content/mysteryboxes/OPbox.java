	...
	public void process() {
		...
		if (player.getInventory().hasItem(new Item(BOX, 1))) {
			...
			if (randomYoda == 0) {
				...
				World.sendMessage("<shad=0>@bla@[@gre@Super Donator box@bla@] [@mag@Legendary@bla@] @gre@" + player.getUsername() + "@bla@ Has just received a @gre@ " + ItemDefinition.forId(godzilla[rewardGrade3][rewardPos]).getName() + " @bla@!");
				...
				if (randomDouble == 0) {
					...
					player.setAnimation(new Animation(6382));
					player.setGraphic(new Graphic(127));
					World.sendMessage("<shad=0>@bla@[@gre@Super Donator box@bla@] [@blu@Uncommon@bla@] @gre@" + player.getUsername() + "@bla@ Has just received a @gre@ " + ItemDefinition.forId(godzilla[rewardGrade3][rewardPos]).getName() + " @bla@!");
					double randomDouble = Misc.random(125);
				...
				player.setAnimation(new Animation(6382));
				player.setGraphic(new Graphic(127));
				World.sendMessage("<shad=0>@bla@[@gre@Super Donator box@bla@] [@gre@common@bla@] @gre@" + player.getUsername() + "@bla@ Has just received a @gre@ " + ItemDefinition.forId(godzilla[rewardGrade3][rewardPos]).getName() + " @bla@!");
				double randomDouble = Misc.random(125);
