...
public class Fishing {
	...
	public enum Spot {
		...
		RAINBOW_FISH(233, new int[]{10138}, 309, 25, new int[]{95}, false, new int[]{29015}, 623);
			...
			public void execute() {
				...
				if (cycle >= Misc.getRandom(1) + reqCycle) {
					...
					if(s.getRawFish()[fishIndex] == 331) {
						p.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.FISH_A_SALMON, 1);

					} else if(s.getRawFish()[fishIndex] == 15270) {
						p.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.FISH_25_ROCKTAILS, 1);
						p.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.FISH_2000_ROCKTAILS, 1);
