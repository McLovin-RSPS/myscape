	...
	public void giveItems() {
		...
		try {
			...
			for (Item item : player.getTrading().offeredItems) {
				PlayerLogs.log(player.getUsername(), "Gave item in trade to "+player2.getUsername()+". Id: "+ item.getDefinition().getName() +", amount: "+item.getAmount());
				}
			for (Item item : player2.getTrading().offeredItems) {
				PlayerLogs.log(player.getUsername(), "Receiving item from trade. Id: "+item.getDefinition().getName() +", amount: "+item.getAmount());
			}
