...
public class BonusManager {
	...
	public static void update(Player player) {
		...
		for (int i = 0; i < STRING_ID.length; i++) {
			...
			int rangedStrength = RangedWeaponData.getRangedStrengthBonus(player);
			player.getPacketSender().sendString(66104, "Ranged Strength: "+rangedStrength);
	...
	public static void sendOtherStats(Player p) {//fixed it now what?
		int magicB = (int) p.getBonusManager().getOtherBonus()[2];//so how do u want this to work? to give set bonus so makes hit harder
		p.getPacketSender().sendString(66105, "Magic Damage: "+magicB+"%");
	}
