...
public final class CombatFactory {
	...
	public static Hit getHit(Character entity, Character victim, CombatType type) {
		switch (type) {
			case MELEE:
				if(victim.isNpc()) {
					if (((NPC) victim).getDefinition().getId() == 5) {
						return new Hit(DesolaceFormulas.calculateMaxMeleeHit(entity, victim), Hitmask.DARK_PURPLE, CombatIcon.MELEE);
					} else {
						return new Hit(RandomUtility.inclusiveRandom(1, /*CombatFactory.calculateMaxMeleeHit(entity, victim)*/ DesolaceFormulas.calculateMaxMeleeHit(entity, victim)), Hitmask.DARK_RED, CombatIcon.MELEE);
					}
				}
			...
			case RANGED:
				if(victim.isNpc()) {
					if (((NPC) victim).getDefinition().getId() == 5) {
						return new Hit(DesolaceFormulas.calculateRangeNew(entity, victim), Hitmask.DARK_PURPLE, CombatIcon.RANGED);

					} else {
						return new Hit(RandomUtility.inclusiveRandom(1, DesolaceFormulas.calculateRangeNew(entity, victim)), Hitmask.DARK_GREEN, CombatIcon.RANGED);
					}
				}
			case MAGIC:
				if(victim.isNpc()) {
					if (((NPC) victim).getDefinition().getId() == 5) {
						return new Hit(DesolaceFormulas.calculateMageNew(entity, victim), Hitmask.DARK_PURPLE, CombatIcon.MAGIC);

					} else {
						return new Hit(RandomUtility.inclusiveRandom(1, DesolaceFormulas.calculateMageNew(entity, victim)), Hitmask.DARK_PURPLE, CombatIcon.MAGIC);
					}
				}
		case DRAGON_FIRE:
	...
	public static int calculateMaxRangedHit(Character entity, Character victim) {
		...
		int rangedStrength = ((int) player.getBonusManager().getAttackBonus()[4] / 1);
	...
	public static boolean checkHook(Character entity, Character victim) {
		...
		if(victim.isNpc() && entity.isPlayer()) {
			...
			if(npc.getLocation() != Location.RAID && npc.getDefinition().getSlayerLevel() > ((Player)entity).getSkillManager().getCurrentLevel(Skill.SLAYER)) {
