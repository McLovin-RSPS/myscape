...
public enum CombatSpecial {
    ...
    BANDOS_MAUL(new int[]{3954}, 60, 1.35, 1.15, CombatType.MELEE, WeaponInterface.WARHAMMER) {
                @Override
                public CombatContainer container(Player player, Character target) {
                    player.performAnimation(new Animation(1667));
                    player.performGraphic(new Graphic(337, GraphicHeight.HIGH));
    ...
    ABYSSAL_WHIP(new int[]{4151, 15441, 15442, 15443, 15444, 20061}, 50, 1, 1, CombatType.MELEE, WeaponInterface.WHIP) {
			  ...
			  RAINBOW_BRUTAL_WHIP(new int[]{20089}, 60, 1.7, 2.4, CombatType.MELEE, WeaponInterface.WHIP) {
                @Override
                public CombatContainer container(Player player, Character target) {
                	int amountToDrain = Misc.getRandom(500);
                    player.performAnimation(new Animation(1658));
                    target.performGraphic(new Graphic(437, GraphicHeight.HIGH));
                    if (target.isPlayer()) {
                        Player p = (Player) target;
                        p.getSkillManager().setCurrentLevel(Skill.STRENGTH, player.getSkillManager().getCurrentLevel(Skill.STRENGTH) - amountToDrain);
                        p.getPacketSender().sendMessage("Your strength has been drained " +amountToDrain+" points from the Brutal spec!");

                    }
                    player.getPacketSender().sendMessage("You drain your opponents strength by a random amount of " +amountToDrain+" strength points");

                    return new CombatContainer(player, target, 1, CombatType.MELEE,
                            false);
                }
            },
			    RAZOR_WHIP(new int[]{17849}, 50, 4.00, 1.2, CombatType.MELEE, WeaponInterface.WHIP) {
                @Override
                public CombatContainer container(Player player, Character target) {
			  ...
			  ZARYTE(new int[]{4706, 18971}, 50, 1.50, 2.87, CombatType.RANGED, WeaponInterface.SHORTBOW) {
  			  ...
  			  ZILYANA_BOW(new int[]{3960}, 55, 1.80, 2.87, CombatType.RANGED, WeaponInterface.SHORTBOW) {
                @Override
                public CombatContainer container(Player player, Character target) {
                    player.performAnimation(new Animation(426));
                    ...
                    TaskManager.submit(new Task(1, player, false) {
                        ...
                        public void execute() {
                            if (tick == 0) {
                                new Projectile(player, target, 660, 44, 3, 32, 31, 0).sendProjectile();
                            } else if (tick >= 1) {
			  ...
			  GUTHIX_BOW(new int[]{19146}, 55, 1.25, 2.50, CombatType.RANGED, WeaponInterface.SHORTBOW) {
                @Override
                public CombatContainer container(Player player, Character target) {
                    player.performAnimation(new Animation(426));
                    ...
                    TaskManager.submit(new Task(1, player, false) {
                        ...
                        public void execute() {
                            if (tick == 0) {
                                new Projectile(player, target, 2340, 44, 3, 32, 31, 0).sendProjectile();
                                new Projectile(player, target, 2340, 60, 3, 32, 31, 0).sendProjectile();
                                new Projectile(player, target, 2340, 76, 3, 32, 31, 0).sendProjectile();
                            } else if (tick >= 1) {
                                target.performGraphic(new Graphic(1729, GraphicHeight.LOW));
                                this.stop();
                            }
                            tick++;
                        }
			  ...
			  ZAMORAK_BOW(new int[]{19149}, 60, 1.75, 2.50, CombatType.RANGED, WeaponInterface.SHORTBOW) {
                @Override
                public CombatContainer container(Player player, Character target) {
                    player.performAnimation(new Animation(426));
                    ...
                    TaskManager.submit(new Task(1, player, false) {
                        ...
                        public void execute() {
                            if (tick == 0) {
                                new Projectile(player, target, 88, 44, 3, 32, 31, 0).sendProjectile();
                            } else if (tick >= 1) {              	
			    ...
			    SARADOMIN_BOW(new int[]{19143}, 50, 1.75, 2.50, CombatType.RANGED, WeaponInterface.SHORTBOW) {
                @Override
                public CombatContainer container(Player player, Character target) {
                    player.performAnimation(new Animation(431));
                    new Projectile(player, target, 280, 44, 3, 32, 31, 0).sendProjectile();
                    
                    return new CombatContainer(player, target, 1, CombatType.RANGED, false) {
                        @Override
    ...
    DRAGONRAGE_BLOWPIPE(new int[] { 3962 }, 50, 1.45, 2.35, CombatType.RANGED, WeaponInterface.DART) {
        		@Override
        		public CombatContainer container(Player player, Character target) {
        			player.performAnimation(new Animation(5061));
        			player.performGraphic(new Graphic(1123));
    ...
    STATIUS_WARHAMMER(new int[]{13902}, 35, 1.3, 1.45, CombatType.MELEE, WeaponInterface.WARHAMMER) {
                ...
                public CombatContainer container(Player player, Character target) {
                    ...
                    };//its somewhere else sec but that would fk up alot of stuff u can just make the bow takes 1% spec
                }
            },
    MAGIC_SHORTBOW(new int[]{937}, 50, 2.00, 2.00, CombatType.RANGED, WeaponInterface.SHORTBOW) {
                @Override
                public CombatContainer container(Player player, Character target) {
    ...
    IDIAN_SHORTBOW(new int[]{4640}, 55, 2.50, 2.50, CombatType.RANGED, WeaponInterface.SHORTBOW) {
                @Override
                public CombatContainer container(Player player, Character target) {
    ...
    DARK_LORDS_BOW(new int[]{3961}, 55, 1.45, 1.65, CombatType.RANGED, WeaponInterface.SHORTBOW) {
                @Override
                public CombatContainer container(Player player, Character target) {
                    player.performAnimation(new Animation(426));
                    ...
                    TaskManager.submit(new Task(1, player, false) {
                        ...
                        public void execute() {
                            if (tick == 0) {
                                ...
                                new Projectile(player, target, 1099, 76, 3, 43, 31, 0).sendProjectile();
                                new Projectile(player, target, 1099, 92, 3, 43, 31, 0).sendProjectile();
                            } else if (tick >= 1) {
                    ...
                    return new CombatContainer(player, target, 4, CombatType.RANGED,
                            true);
                }
            },
    ...
    ICY_HALBERD(new int[]{3204}, 30, 1.07, 1.08, CombatType.MELEE, WeaponInterface.HALBERD) {
                @Override
                public CombatContainer container(Player player, Character target) {
    ...
    ICY_CLAWS(new int[]{5168}, 10, 3, 4.4, CombatType.MELEE, WeaponInterface.CLAWS) {
                @Override
                public CombatContainer container(Player player, Character target) {
    ...
    public static void drain(Player player, int amount) {
        ...
        player.getAchievementTracker().progress(com.arlania.world.content.achievements.AchievementData.PERFORM_A_SPECIAL_ATTACK, 1);
    }
