...
public class Dungeoneering {
	...
	public static void start(final Player p) {
		...
		RegionInstance partyInstance = new RegionInstance(party.getOwner(), RegionInstance.RegionInstanceType.RAID)
;		final int height = p.getIndex() * 4;
		...
		for(Player member : party.getPlayers()) {
			if(member != null) {
				...
				member.getClickDelay().reset(); //nvm
	...
	public static void setupFloor(DungeoneeringParty party, int height) {
		...
		System.out.println("Spaned all objects");
	}
