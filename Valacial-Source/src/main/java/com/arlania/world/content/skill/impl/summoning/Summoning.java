...
public class Summoning {
	...
	public void summon(final FamiliarData familiar, boolean renew, boolean login) {
		...
		if(!player.getLocation().isSummoningAllowed()) {
			player.getPacketSender().sendMessage("You cannot summon monsters here.");
			return;
		}
		...
		if (login || player.getSkillManager().getMaxLevel(Skill.SUMMONING) >= familiar.levelRequired) {
			...
			if(!login) {
				if (player.getSkillManager().getCurrentLevel(Skill.SUMMONING) >= familiar.summoningPointsRequired) {
					...
					if(renew && getFamiliar() != null) {
						...
						getFamiliar().getSummonNpc().performGraphic(new Graphic(1738));
			...
			foll.performGraphic(new Graphic(1738));
	...
	public void summonPet(BossPet bossPet, boolean login) {
		...
		if(getFamiliar() != null && !login) {
			player.getPacketSender().sendMessage("You already have a monster.");
			return;
		}
		if(!login) {
			unsummon(true, false);
	...
	public void handleUpdate() {
		if(getFamiliar() !=null) {
			player.getInventory().add(player.getBosspets().getItemId(), 1);
			player.setBosspets(null);
			player.getPacketSender().sendString("@gre@"+ NPCDrops.getDroprate(player,true)+"%",37469);
			player.getPacketSender().sendString("@gre@"+ NPCDrops.getDoubleDr(player,true)+"%",37470);
		...
		if(dropItems) {
			if(bob != null) {
				if(bob.getValidItems().size() > 0) {
					...
					player.getPacketSender().sendMessage("Your Monster has dropped its carried items on the floor.");
				}
				bob = null;
			}
		}
		if(getFamiliar() != null && getFamiliar().getSummonNpc() != null) {
			...
			if(full) {
				if(BossPet.forSpawnId(spawnId) != null) {
					player.giveItem(BossPet.forSpawnId(spawnId).getItemId(), 1);
					player.getPacketSender().sendString("@gre@"+ NPCDrops.getDroprate(player,true)+"%",37469);
					player.getPacketSender().sendString("@gre@"+ NPCDrops.getDoubleDr(player,true)+"%",37470);
					...
					if(clockTimer >= 2) {
						if(familiar.getDeathTimer() > 0) {
							...
							if(familiar.getDeathTimer() == 60)
								player.getPacketSender().sendMessage("@red@Warning: Your monster will fade away in one minute.");
							...
							unsummon(true, true);
							stop();
							player.getPacketSender().sendMessage("Your monster has died.");
						}
						clockTimer = 0;
					}
					clockTimer++;
	...
	public void store() {
		...
		if(getFamiliar() != null && getFamiliar().getSummonNpc() != null && bob != null)
			...
			player.getPacketSender().sendMessage("You do not have a monster which can hold items.");
	}
	...
	public void toInventory() {
		...
		if((!player.busy() || player.getInterfaceId() == BeastOfBurden.INTERFACE_ID) && player.getLocation().isSummoningAllowed()) {
			...
			if(bob.getValidItems().size() == 0) {
				player.getPacketSender().sendMessage("Your monster is not currently holding any items for you.");
				return;
			}
	...
	public void moveFollower(boolean forced) {
		if(getFamiliar() != null && getFamiliar().getSummonNpc() != null) {
			...
			if(forced || canSpawn(getFamiliar().getSummonNpc(), movePos)) {
				...
				player.getLastSummon().reset();
				if(!forced) {
					player.getPacketSender().sendMessage("You've called your monster.");
				}
			} else {
				getFamiliar().getSummonNpc().moveTo(movePos);
				getFamiliar().getSummonNpc().performGraphic(new Graphic(1738));
	...
	public void login() {
		...
		if(getFamiliar() != null && !player.getInventory().isFull()) {
			player.getInventory().add(player.getBosspets().getItemId(), 1);
			System.err.println("Added pet to the inventory.");
		} 
		spawnTask = null;
	}
