...
public class Construction {
	...
	public static String hasReqs(Player p, Furniture f, HotSpots hs) {
		...
		if (p.getSkillManager().getCurrentLevel(Skill.PVMING) < f.getLevel()) {
				...
				if (pf.getRoomX() == myTiles[0] - 1 && pf.getRoomY() == myTiles[1] - 1) {
					if (pf.getHotSpot(p.getHouseRooms()[p.inConstructionDungeon() ? 4 : p.getPosition().getZ()][myTiles[0] - 1][myTiles[1] - 1].getRotation()) == hs) {
						if (pf.getFurnitureId() != fur.getFurnitureId()) {
							return "This is an Upgrade piece of furniture. (build the furniture before this first)";
						}
					}
				}
			}
	...
	public static boolean buildActions(Player p, Furniture f, HotSpots hs) {
		...
		p.getSkillManager().addExperience(Skill.PVMING, f.getXP() * 50);
		return true;
	}
	...
	public static boolean handleButtonClick(int buttonId, Player p) {
		...
		case 2471:
			...
			break;
//			case DissolveHandler.UNIQUE_UID:
//				DissolveHandler.handleConfirm(p, p.dissolveId, p.dissolveOrbAmount, p.dissolveXP, null);
//				break;
		...
		case 2461:
			...
			if (p.getDialogueActionId() == 444) {
				...
				if (p.getSkillManager().getCurrentLevel(Skill.PVMING) < b.getConsLevel()) {
	...
	public static void createRoom(int roomType, Player p, int toHeight) {
		...
		if (rd.getLevelToBuild() > p.getSkillManager().getCurrentLevel(Skill.PVMING)) {
