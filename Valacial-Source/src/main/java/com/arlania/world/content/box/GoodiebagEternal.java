...
public class GoodiebagEternal {

    private Player player;

    public GoodiebagEternal(Player player) {
        this.player = player;
    }
    ...
    public int boxId = 3570;//3578

    public int[] rewards = {20551, 6821, 6821, 6821, 6821, 6821, 6821, 6821, 6821, 6821, 6821, 6821, 6821, 6821, 6821, 6821, 6821, 6821, 6821, 6821};
    ...
    public void open() {
        player.setLastgoodiebox(3570);
        player.getPacketSender().sendString(49210, "19x Eternal Stones\\\\n1x Jackpot");
        player.getPacketSender().sendString(49202, "ImaginePS GoodieBox @cya@(Eternal)");
    ...
    public void claim() {
        ...
        if (!claimed) {
            if (player.getInventory().contains(boxId)) { // gg this is guaranteed to work
                ...
                String announcement = "@red@[Eternal GoodieBox]@bla@ " + player.getUsername() + "@red@ has @bla@" +ItemDefinition.forId(rewards[player.selectedGoodieBag]).getName() +"@red@ as reward!";
                World.sendMessage(announcement);
                claimed = true;
                boxId = 3570;
            } else {
                player.sendMessage("@red@You need a Eternal goodiebox to claim the reward");
            }
        } else {
