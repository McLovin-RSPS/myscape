...
public class GamblingManager extends PacketInteraction {
	...
	public static void sendInvitation(final Player player, Player otherPlayer) {
		Player requested = otherPlayer.getGambling1().getRequested();
		if (!Objects.isNull(requested)) {
			...
			player.getGambling1().setRequested(otherPlayer);
			player.getGambling1().setStage(GambleStage.SENDING_OFFER);
		}
	}
	...
	public static void acceptInvitation(final Player player, Player otherPlayer) {
		 ...
		 */
		if (otherPlayer.getGambling1().getRequested() != null) {
			/**
			 * We are the requested one
			 */
			if (player.getIndex() == otherPlayer.getGambling1().getRequested().getIndex()) {
				/**
	...
	private static void acceptGameSelection(Player player) {
		 ...
		 * No game selected
		 */
		if (player.getGambling1().getType() == null) {
		 ...
		 */
		Player otherPlayer = player.getGambling1().getRequested();
		/**
		 ...
		 * Already confirmed
		 */
		if (player.getGambling1().isConfirmed()) {
			return;
		}
		/**
		 * Confirms selection
		 */
		player.getGambling1().setConfirmed(true);
		 ...
		 * No game selected
		 */
		if (player.getGambling1().getType() != otherPlayer.getGambling1().getType()) {
			return;
		}
		/**
		 * Other player confirmed
		 */
		if (otherPlayer.getGambling1().isConfirmed()) {
			/**
			 * The game the other player is offering
			 */
			GamblingGame game = getGame(otherPlayer, player, otherPlayer.getGambling1().getType());
			/**
			 * Sets the game
			 */
			player.getGambling1().setGame(game);
			otherPlayer.getGambling1().setGame(game);
			/**
			 * Sets stage for us
			 */
			player.getGambling1().setStage(GambleStage.SENDING_OFFER);
	...
	public static void openOfferScreen(final Player player, Player otherPlayer) {
		 ...
		 * Sets the stage
		 */
		player.getGambling1().setConfirmed(false);
		player.getGambling1().setStage(GambleStage.PLACING_BET);
		/**
		 * Displays items
		 */
	...
	public static void startGamble(Player player) {
		...
		if (otherPlayer.getGambling1().isConfirmed()) {
			 ...
			 * No game available
			 */
			if (player.getGambling1().getGame() == null) {
				return;
			}
			/**
			 * No host
			 */
			if (player.getGambling1().getGame().getHost() == null) {
				return;
			}
			/**
			 * Host doesn't have a game
			 */
			if (player.getGambling1().getGame().getHost().getGambling1().getGame() == null) {
				return;
			}
			/**
			 * Removes interfaces
			 ...
			 * Sets the stage
			 */
			player.getGambling1().setStage(GambleStage.IN_PROGRESS);
			otherPlayer.getGambling1().setStage(GambleStage.IN_PROGRESS);
			/**
			 * Movement locked
			 */
			 ...
			 * The gamble
			 */
			player.getGambling1().getGame().gamble();
		}
	}
	...
	public static void finishGamble(int gameId, Player host, Player opponent, int hostScore, int opponentScore) {
		 ...
		 * No game available
		 */
		if (host.getGambling1().getGame() == null) {
			return;
		}
		/**
		 * No host
		 */
		if (host.getGambling1().getGame().getHost() == null) {
			return;
		}
		/**
		 * Host doesn't have a game
		 */
		if (host.getGambling1().getGame().getHost().getGambling1().getGame() == null) {
			return;
		}
		/**
		 * Opponent invalid
	...
	public static void selectGameType(Player player, GameType type) {
		 ...
		 */
		player.getGambling1().setConfirmed(false);
		player.getGambling1().setType(type);
		player.getPacketSender().sendString(30910, type.description);

		otherPlayer.getGambling1().setConfirmed(false);
		otherPlayer.getGambling1().setType(type);
	...
	public static void resetGamble(final Player player) {
		 ...
		 * Not in gamble
		 */
		if (player.getGambling1().getStage().equals(GambleStage.OFFLINE)) {
			return;
		}
		/**
		 * Gamble settings
		 */
		player.getGambling1().setConfirmed(false);
		player.getGambling1().setGame(null);
		player.getGambling1().setType(null);
		player.getGambling1().setStage(GambleStage.OFFLINE);
		player.getGambling1().setSelection(-1);
		 ...
		 * Resets other player
		 */
		if (player.getGambling1().getRequested() != null) {
			resetGamble(player.getGambling1().getRequested());
		}
		/**
		 * Resets request
		 */
		player.getGambling1().setRequested(null);
	}
	...
	private static boolean canGamble(final Player player, GambleStage requiredStage) {
		...
		if (Objects.nonNull(requiredStage)) {
			 ...
			 * Wrong stage
			 */
			if (!player.getGambling1().getStage().equals(requiredStage)) {
				return false;
			}
		}
		/**
	...
	private static boolean illegalGamble(Player player) {
		...
		for (Player players : World.getPlayers()) {
			...
			if (players.getGambling1().getRequested() == player
					&& !players.getGambling1().getStage().equals(GambleStage.OFFLINE)) {
				amount++;
			}
		}
		return amount > 2;
	...
	public static boolean handleGambleOffer(Player player, int interfaceId, Item item, int slot) {
		...
		case OFFER_INVENTORY:
			...
			player.getPacketSender().sendString(28349, "");
			if (player.getGambling1().getRequested() != null) {
				player.getGambling1().getRequested().getPacketSender().sendString(28349, "");
			}
			if (otherPlayer != null) {
	...
	private static void selectOption(Player player, int option) {
		 ...
		 * Sets the selection
		 */
		player.getGambling1().setSelection(option);
		/**
		 * Black jack
		 */
		if (player.getGambling1().getGame().getHost().getGambling1().getGame() instanceof BlackJackGamblingGame) {
			/**
			 * Second option
			 */
			if (option == 2) {
				player.getGambling1().getGame().getHost().getGambling1().getGame().setHostTurn(true);
				player.getGambling1().getGame().setHostTurn(true);
				if (player.getGambling1().getGame().getHost() != player) {
	...
	private static void removeFlowers(Player player) {
		 ...
		 */
		for (GameObject gameFlower : player.getGambling1().getGameFlowers()) {
			/**
