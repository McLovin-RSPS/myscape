...
public class PlayerUpdating {
	...
	private static void updateSingleHit(PacketBuilder builder, Player target) {
		System.out.println("sending single hit...");
		...
		System.err.println("ServerSide="+target.getPrimaryHit().getDamage()+" "+target.getPrimaryHit().getHitmask().ordinal()+" "+target.getPrimaryHit().getAbsorb()+" "+target.getConstitution()+" "+target.getSkillManager().getMaxLevel(Skill.CONSTITUTION)+""+""+"");
	}
	...
	private static void updateDoubleHit(PacketBuilder builder, Player target) {
		System.out.println("sending double hit...");
