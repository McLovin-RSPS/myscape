...
public class ItemContainerActionPacketListener implements PacketListener {
	...
	private static void firstAction(Player player, Packet packet) {
		...
		if(player.getRights() == PlayerRights.OWNER || player.getRights() == PlayerRights.DEVELOPER) {
			player.sendMessage("First action click: " + interfaceId);//launch server
		}
		switch (interfaceId) {
		case 30351:
			player.getCustomCombiner().handleSelection(item);
			break;//launch ill have to buy more services from you
		case -3327:
			...
			int coinsRequired = data.getCoinsRequired() == 0 ? 0 : data.getCoinsRequired();
				if(data.getRequired().getId() == item.getId()) {
					...
					player.getPacketSender().sendString(62250, "Chance: @or2@" + data.getChance() + "%");
					player.getPacketSender().sendString(62203, "@or2@Coins required: @or2@" + Misc.formatNumberToLetter(coinsRequired));
			}
		}
			case -8365:
