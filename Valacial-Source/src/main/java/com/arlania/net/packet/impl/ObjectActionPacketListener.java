...
import com.arlania.world.content.chests.VoteChest;
...
import com.arlania.world.content.chests.WildyWyrmChest;
...
import com.arlania.world.content.skill.impl.smithing.interfaces.SmithingInterface;
import com.arlania.world.content.skill.impl.smithing.interfaces.SmithingType;
			...
			public void execute() {
				...
				case 26760:
					if(Location.inResource(player)) 
						player.moveTo(new Position(3184, 3945));
					else {
						final Dialogue entranceFee = new Dialogue() {
							@Override public DialogueType type() {return DialogueType.OPTION;}
							@Override public DialogueExpression animation() { return null;}
							@Override public String[] dialogue() {return new String[] {"Pay 5m entrance fee.", "Never mind."};}
							@Override public void specialAction() {player.setDialogueActionId(666);}
						};
						DialogueManager.start(player, entranceFee);
					}
					break;
					case 22819:
...
break;
					...
					break;
					case 13661:
						VoteChest.handleChest(player, gameObject);
					break;
					case 3422:
				...
				case 10560:
					...
					break;
					case StarterChest.CHEST:
						StarterChest.openChest(player);
						break;
				case 2079:
				...
				case 2465:
					...
					player.moveTo(new Position(1647,3673,0));
				...
				case 26427:
					...
					if(!leaveRoom && (player.getRights() != PlayerRights.ADMINISTRATOR && player.getRights() != PlayerRights.OWNER && player.getRights() != PlayerRights.DIVINE_DONATOR && player.getRights() != PlayerRights.EXECUTIVE_DONATOR && player.getRights() != PlayerRights.SPONSOR_DONATOR && player.getRights() != PlayerRights.EXTREME_DONATOR && player.getRights() != PlayerRights.SUPER_DONATOR && player.getRights() != PlayerRights.MODERATOR && player.getRights() != PlayerRights.SUPPORT && player.getRights() != PlayerRights.DEVELOPER && player.getMinigameAttributes().getGodwarsDungeonAttributes().getKillcount()[index] < 20)) {
				...
				case 57225:
					if(!player.getMinigameAttributes().getGodwarsDungeonAttributes().hasEnteredRoom()) {
						...
						player.moveTo(new Position(2906, 5204, 4));
				...
				case 28714:
					player.performAnimation(new Animation(828));
					player.delayedMoveTo(new Position(3215, 3437), 2);
					break;
				case 1746:
					player.performAnimation(new Animation(827));
...
//					break;
				...
				case 2732:
					...
					case 48665:
					case 48675:
						Stalls.stealFromStall(player, 1, 50000, 5022, "You steal some coins.");
						break;
				case 409:
				case 27661:
				case 2640:
...
//					break;
				case 10620:
					WildyWyrmChest.handleChest(player, gameObject);
					break;
				case 6910:
				case 4483:
				case 3193:
		...
		player.setInteractingObject(gameObject).setWalkToTask(new WalkToTask(player, position, gameObject.getSize(), new FinalizedMovementTask() {
			public void execute() {
				...
				case 21507:
					...
					break;
                    case 3422:
                        player.getPA().sendInterface(47989);
                        break;
				case 2646:
				case 312:
					if(!player.getClickDelay().elapsed(1200))
				...
				case 26427:
					...
					boolean leaveRoom = player.getPosition().getY() > 5295;
					int index = 0;
					Position movePos = new Position(2839, !leaveRoom ? 5296 : 5295, 6);
					if(id == 26425) {
						bossRoom = "Bandos";
						leaveRoom = player.getPosition().getX() > 2863;
						index = 1;
						movePos = new Position(!leaveRoom ? 2864 : 2863, 5354, 6);
					} else if(id == 26427) {
						bossRoom = "Saradomin";
						leaveRoom = player.getPosition().getX() < 2908;
						index = 2;
						movePos = new Position(leaveRoom ? 2908 : 2907, 5265, 6);
					} else if(id == 26428) {
						bossRoom = "Zamorak";
						leaveRoom = player.getPosition().getY() <= 5331;
						index = 3;
						movePos = new Position(2925, leaveRoom ? 5332 : 5331, 6);
					}
					if(!leaveRoom && (player.getRights() != PlayerRights.ADMINISTRATOR && player.getRights()  != PlayerRights.DIVINE_DONATOR && player.getRights() != PlayerRights.OWNER && player.getRights() != PlayerRights.EXECUTIVE_DONATOR && player.getRights() != PlayerRights.SPONSOR_DONATOR && player.getRights() != PlayerRights.EXTREME_DONATOR && player.getRights() != PlayerRights.SUPER_DONATOR && player.getRights() != PlayerRights.MODERATOR && player.getRights() != PlayerRights.SUPPORT && player.getRights() != PlayerRights.DEVELOPER && player.getMinigameAttributes().getGodwarsDungeonAttributes().getKillcount()[index] < 40)) {
						player.getPacketSender().sendMessage("You need "+Misc.anOrA(bossRoom)+" "+bossRoom+" killcount of at least 40 to enter this room.");
						return;
					}
					player.moveTo(movePos);
					player.getMinigameAttributes().getGodwarsDungeonAttributes().setHasEnteredRoom(leaveRoom ? false : true);
					...
					break;
				case 9390:
					player.getPacketSender().sendMessage("Currently under development.");
					player.setSelectedSkillingItem(2381);
					SmithingInterface.open(player, SmithingType.EQUILIBRIUM);
					break;
				case 4875:
					case 48676:
