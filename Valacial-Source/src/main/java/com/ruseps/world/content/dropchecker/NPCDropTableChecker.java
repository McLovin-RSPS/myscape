...
public class NPCDropTableChecker {
	...
	public boolean handleButtonClick(final Player player, final int actionId) {
		if (actionId >= -27885 && actionId <= -27716) {
			...
			// int secondLeft = (int) (1200 - (System.currentTimeMillis() -
			// player.npcDropTableDelay)) / 1000;
			// player.getPacketSender().sendMessage("You need to wait "+secondLeft+"
			// second(s) before checking another drop table.");
			// return true;
			// }
			...
			player.getPacketSender().sendFrame126(
					"@or1@You are now viewing the @whi@" + dropTableNpcNames.get(index) + "@or1@ drop table.", 37605);
	...
	public void showNPCDropTable(final Player player, final int npcId) {
		...
		for (Entry<DropChance, List<Item>> entry : dropTables.entrySet()) {
			...
			if (chance == DropChance.ALWAYS) {
			...
			} else if (chance == DropChance.LEGENDARY || chance == DropChance.LEGENDARY) {
				...
				player.getPacketSender().sendItemsOnInterface(37923, 60, dropTable, false);
			}
		}
